...
local fpftheme_layer = collision_util.get_first_unused_layer()

-- disable power options
local function make_unplaceable(entity)
  entity.collision_mask = {"item-layer", "object-layer", "player-layer", "water-tile", "resource-layer" , fpftheme_layer, "ground-tile", "doodad-layer", "floor-layer", "rail-layer", "transport-belt-layer","ghost-layer", "train-layer" }      
end

local alternatePowerEnabled = settings.startup["fpftheme-dont-disable-alternate-power"].value
...
local oilProcessingUseWater = settings.startup["fpftheme-dont-change-oil-recipes"].value
...
local effectivityModuleNotAltered = settings.startup["fpftheme-dont-change-effectivity-modules"].value
